...
Phase 3 is where the platform starts understanding exam _structure_:
...
- **Ambiguity can't always be resolved locally.** `"(i)"` looks like a
  valid single-letter sub-part _and_ a roman numeral — resolved by
  knowing Cambridge conventions (sub-parts never reach `i`/`v`/`x`).
  `"A load is fixed to trolley P."` is _textually identical_ to a real
...
- **A statistic can be wrong in a way that still mostly works.** Using
  the _most common_ line-start position as "the margin" passed every
...
- **Diagram-positioned MCQ options.** Some multiple-choice questions
  ...
  **Measured in Phase 5:** 11 of 80 multiple-choice questions in the
  June 2024 session reach the vector store with no options at all —
  20% of paper 21. The worst embeds as "Which diagram shows waves
  diffracting? A B D". They are flagged `isExemplar: false` so they
  stay searchable but are never shown to a generator as a model.
- **Single board, single subject.** Everything is validated against
  ...
  OCR, or IB formats.
- ~~**Mark schemes aren't parsed.**~~ **Closed in Phase 4** —
  `mark-scheme-extraction` and `examiner-report-extraction`.
- ~~**No answer-key extraction.**~~ **Closed in Phase 4** —
  `KnowledgeQuestion.correctAnswer`, from the mark scheme.
- **Front-matter is silently dropped.** Instructions, cover-page text,
  ...
  changes.
- ~~**Back matter was silently kept.**~~ The opposite gap, unnoticed
  until Phase 5: the copyright acknowledgement printed after the last
  question was absorbed into it. **Closed** by `stripBackMatter`.
- ~~**Withdrawal notices were read as questions.**~~ A board that
  withdraws a question leaves "the question has been removed from the
  question paper" in its slot, keeping the numbering, and every
  structural rule here treated that as a question. **Closed** by
  `isWithdrawnNotice`, conservatively: it will not mark anything
  carrying sub-parts or options.
