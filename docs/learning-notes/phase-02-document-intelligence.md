...
- ~~**No reading order.**~~ **Closed in Phase 3** by `reconstructLines`,
  exactly where this note predicted it would need closing.
...
- ~~**Tables expose geometry, not content.**~~ **Closed** —
  `TableElement.cells` carries cell text, added when mark-scheme
  extraction needed it.
- **Back matter is not separated from content.** Boards print a
  copyright acknowledgement after the last question in the same text
  flow with no structural marker, so it was absorbed into the final
  question of every paper. Phase 5 found it by measuring pairwise
  similarity across the corpus and now strips it in
  `stripBackMatter` — but the underlying gap is that nothing here
  knows where a paper stops being a paper.
