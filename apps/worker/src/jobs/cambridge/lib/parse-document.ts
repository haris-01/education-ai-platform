...
function classifyType(title: string): DocumentType {
  if (/mark scheme/i.test(title)) return "MARK_SCHEME";
  if (/examiner report/i.test(title)) return "EXAMINER_REPORT";
  if (/confidential instructions/i.test(title)) return "CONFIDENTIAL_INSTRUCTIONS";
  if (/specimen/i.test(title)) return "SPECIMEN_PAPER";
  if (/question paper/i.test(title)) return "QUESTION_PAPER";
...
function extractPaperAndVariant(title: string): { paper?: number; variant?: number } {
