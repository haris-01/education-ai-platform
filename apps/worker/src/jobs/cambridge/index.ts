...
export async function collectCambridgeSubject(
  subjectUrl: string
): Promise<void> {
  ...
  logger.info(
    `Processing ${resources.length} document(s) for ${subject.subject} (${subject.syllabusCode})`
  )

  // Sequential on purpose: downloads hit the same exam-board server, and one
  // resource failing shouldn't abort the rest — a mapped Promise.all would
  // fire every request concurrently and fail the whole batch on one error.
