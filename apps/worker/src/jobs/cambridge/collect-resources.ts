...
async function collectFromSection(
  ...
  // Inline (rather than a standalone function) so the element parameter's
  // type is inferred from `.map()` — cheerio doesn't publicly export the
  // underlying node type to annotate it by hand.
  return $('a[href]')
    .map((_index, element): DocumentResource | undefined => {
