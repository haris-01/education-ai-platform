...
const SECTION_LABEL_MAP: Array<{ match: RegExp; key: keyof SubjectSections }> = [
  ...
  { match: /published resource|endorsed resource/i, key: "publishedResources" },
 ...
 */
export async function collectSubject(subjectUrl: string): Promise<SubjectMetadata> {
...
function discoverSections($: cheerio.CheerioAPI, subjectUrl: string): SubjectSections {
  ...
  $(".landingLinks.within-context a").each((_, element) => {
    ...
    sections[entry.key] = `${basePath}${href}/`;
  });
