...
function buildPaperReport(
  ...
  const questionCommentsSection = sectionAfter(
    lines,
    SPECIFIC_QUESTIONS_HEADING
  )
...
function buildQuestionComments(lines: Line[]): QuestionComment[] {
  const drafts = lines.reduce<QuestionCommentDraft[]>((acc, line) => {
    ...
    if (headingMatch) {
      return [
        ...acc,
        { questionNumber: Number(headingMatch[1]), textParts: [] },
      ]
    }
  ...
  return drafts.map((draft) => {
    const comment = draft.textParts.join(' ').trim()
    return {
      questionNumber: draft.questionNumber,
      comment,
      commonMistakes: extractCommonMistakes(comment),
    }
  })
}
