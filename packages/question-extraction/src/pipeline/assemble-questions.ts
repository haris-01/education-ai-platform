...
function applyQuestionNumber(
  ...
  const current: QuestionBuilder = {
    ...
    marks:
      classified.totalMarks ?? (currentPart ? undefined : classified.marks),
    pageNumbers: [pageNumber],
    parts: [],
    currentPart,
    options: [],
...
function applySubSubpart(
  ...
  if (
    ...
    classified.partLabel === undefined
  ) {
    return state
  }
...
function finalizeQuestion(rawQuestion: QuestionBuilder): Question {
  ...
  const parts = question.currentPart
    ? [...question.parts, finalizePart(question.currentPart)]
    : question.parts
  const text = stripBackMatter(question.textParts.join(' ').trim())

  return {
    number: question.number,
    text,
    withdrawn: isWithdrawnNotice(
      text,
      parts.length > 0,
