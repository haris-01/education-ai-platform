...
const INLINE_OPTIONS_PATTERN = /^A\s+(.*?)\s+B\s+(.*?)\s+C\s+(.*?)\s+D\s+(.*)$/
...
function classifyLine(line: Line, leftMargin: number): ClassifiedLine[] {
  ...
  if (isBoilerplate(text, line)) {
    return [
      buildClassifiedLine(line, 'boilerplate', marks, totalMarks, text, {}),
    ]
  }
  ...
  if (questionMatch && isNearMargin) {
    ...
    const embeddedSubSubpartMatch =
      afterEmbeddedSubpart?.match(SUB_SUBPART_PATTERN)
    const content =
    ...
    return [
      buildClassifiedLine(line, 'questionNumber', marks, totalMarks, content, {
