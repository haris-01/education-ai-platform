...
describe.skipIf(!datasetAvailable)('question extraction corpus', () => {
      ...
      questions.forEach((q) => {
        ...
        if (q.marks !== undefined && q.parts.length > 0) {
          const sum = q.parts.reduce((partSum, part) => {
            ...
            return (
              partSum +
              part.subParts.reduce(
                (subSum, sub) => subSum + (sub.marks ?? 0),
                0
              )
            )
          }, 0)
  ...
  // Every paper prints its own code, so the expected value here is read
  // off the dataset path rather than hand-listed: past papers live under
  // their variant directory ("past-papers/2024/mj/41/..."), specimen
  // papers under the bare paper number ("specimen-papers/2023/4/...")
  // because they carry no variant.
  it.each(QUESTION_PAPERS)(
    'identifies the paper %s belongs to',
    async (relativePath) => {
      const expectedCode = relativePath.split('/').at(-2)
      const filePath = path.join(DATASET_ROOT, relativePath)
      const parsed = await parseNativePdf(filePath)
      const { metadata } = buildQuestionDocument(parsed)

      expect(metadata.paper).toBeDefined()
      expect(metadata.paper?.syllabusCode).toBe('0625')
      expect(metadata.paper?.code).toBe(expectedCode)
      expect(metadata.paper?.number).toBe(Number(expectedCode?.charAt(0)))
    },
    30000
  )
})
